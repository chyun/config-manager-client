package com.chyun.config.manager.client.test;
...
public class CuratorTest {
    ...
    public void get() throws Exception {
        String path = "/register"; //String path = "/zk/test";
        ...
        //添加session过期的监控
        addReconnectionWatcher(path, ZookeeperWatcherType.GET_CHILDREN, watch);
    }

    public void register() throws Exception {
        String ip = InetAddress.getLocalHost().getHostAddress();
        String registeNode = "/register/" + ip;//节点路径

        byte[] data = "disable1".getBytes(charset);//节点值
        ...
        if (stat != null) {
            data = "disable2".getBytes(charset);//节点值
            zkTools.setData().forPath(registeNode, data);
        } else {
            zkTools.create()
